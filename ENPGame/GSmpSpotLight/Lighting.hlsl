...
cbuffer cb2: register(b2)
{
	...
	//float			    g_fDamping[g_iNumLight] : packoffset(c4.w);
	float4				g_vLightPos[g_iNumLight];
	//float			    g_fRadius[g_iNumLight] : packoffset(c5.w);
	float4				g_vEyeDir[g_iNumLight];
	//float			    g_fIntensity[g_iNumLight] : packoffset(c6.w);
	float4				g_vEyePos[g_iNumLight];
	//float			    g_fEyeRadius[g_iNumLight] : packoffset(c7.w);
};
//--------------------------------------------------------------------------------------
//Lighting Variables
//--------------------------------------------------------------------------------------
...
float4 PS(VS_OUTPUT vIn) : SV_Target
{
	...
	float4 vFinalColor = ComputeSpotLight(vIn.v, vIn.n, g_iNumLight) *vTexColor;
	return vFinalColor;
}
//--------------------------------------------------------------------------------------
// Pixel Shader
