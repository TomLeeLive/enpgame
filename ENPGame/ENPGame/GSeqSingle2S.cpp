...
GSeqSingle2S * GSeqSingle2S::pInstance_ = 0;
//
//
//GSeqSingle2S::GSeqSingle2S()
//{
//}
//
//
...
bool GSeqSingle2S::Frame() {
	...
	((GSeqSinglePlay*)g_pMain->m_pGameSeq[G_SEQ_SINGLE])->m_pFPSCamera[G_HERO_TOM].get()->SetViewMatrix(D3DXVECTOR3(
		I_GameEventMgr.m_vecStage[1]->m_vPosStart.x,
		G_DEFINE_HERO_1_POS_Y,
		I_GameEventMgr.m_vecStage[1]->m_vPosStart.z)
		, D3DXVECTOR3(-10.0f, 10.0f, 50.0f));

	((GSeqSinglePlay*)g_pMain->m_pGameSeq[G_SEQ_SINGLE])->m_GAIColMgr.m_Zomb[0]->m_matWorld._41 = I_GameEventMgr.m_vecStage[1]->m_vPosStart.x + 100.0f;
	((GSeqSinglePlay*)g_pMain->m_pGameSeq[G_SEQ_SINGLE])->m_GAIColMgr.m_Zomb[0]->m_matWorld._42 = G_DEFINE_HERO_1_POS_Y;
	((GSeqSinglePlay*)g_pMain->m_pGameSeq[G_SEQ_SINGLE])->m_GAIColMgr.m_Zomb[0]->m_matWorld._43 = I_GameEventMgr.m_vecStage[1]->m_vPosStart.z;
	...
	for (int i = 0; i < I_GameEventMgr.m_vecStage[1]->m_vecZombiePos.size(); i++) {
		((GSeqSinglePlay*)g_pMain->m_pGameSeq[G_SEQ_SINGLE])->m_GAIZombMgr.Load(G_DEFINE_ZOMBIE_ADD_UNIT, I_GameEventMgr.m_vecStage[1]->m_vecZombiePos[i]);
	}

	//싱글 플레이로 스페이스 키 눌렀을 때 시퀀스변경.//m_iSelected =1 으로 돌릴것
	...
	if (I_Input.KeyCheck(DIK_SPACE) == KEY_PUSH) {
		...
		((GSeqSinglePlay*)g_pMain->m_pGameSeq[G_SEQ_SINGLE])->m_MapMgr.m_iMapSelected = 1;
...
bool GSeqSingle2S::Render() {
	//render
	m_UIManager.Render();
	return true;
};
bool GSeqSingle2S::Release() {
	return true;
};