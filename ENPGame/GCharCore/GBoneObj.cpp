...
void GBoneObj::SetBoneMatrices(ID3D11DeviceContext*    pContext)
{
...
void GBoneObj::SetBoneMatrices(ID3D11DeviceContext*    pContext, D3DXMATRIX* pMatrix)
{
	D3DXMATRIX* pMatrices;
	HRESULT hr = S_OK;
	D3D11_MAPPED_SUBRESOURCE MappedFaceDest;
...
void GBoneObj::SetBoneMatrices(ID3D11DeviceContext*    pContext, D3DXMATRIX* pMatrix, vector<D3DXMATRIX>* pList)
{
	D3DXMATRIX* pMatrices;
	HRESULT hr = S_OK;
	D3D11_MAPPED_SUBRESOURCE MappedFaceDest;
	...
	vector<D3DXMATRIX>* pList)
{
	...
	HRESULT hr = S_OK;
	D3D11_MAPPED_SUBRESOURCE MappedFaceDest;
	if (SUCCEEDED(pContext->Map((ID3D11Resource*)pBoneBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &MappedFaceDest)))
	{
		pMatrices = (D3DXMATRIX*)MappedFaceDest.pData;
		for (DWORD dwObject = 0; dwObject < m_Scene.iNumMesh; dwObject++)
		{
...
bool GBoneObj::Convert(const TCHAR* strPathName)
{
	return true;
}
void GBoneObj::SetBoundBox(D3DXVECTOR3* Quad, tMatMesh* pMesh)
{
...
HRESULT GBoneObj::SetInputLayout()
{
	// 레이아웃 생성
...
bool GBoneObj::SetBuffer(ID3D11Device* pd3dDevice)
{
...
void GBoneObj::SetMatrix(D3DXMATRIX* pWorld, D3DXMATRIX* pView, D3DXMATRIX* pProj)
{
	if (pWorld != NULL)
	{
		m_matWorld = *pWorld;
...
bool GBoneObj::CombineBuffer(ID3D11Buffer* pVB, ID3D11Buffer* pIB)
{
	size_t dstOffset = 0;
	size_t vbOffset = 0;
...
bool GBoneObj::Draw(ID3D11DeviceContext*    pContext, GModel* pParent)
{
