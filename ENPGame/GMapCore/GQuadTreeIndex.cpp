...
bool GQuadTreeIndex::Build( GMap* pMap, DWORD dwWidth, DWORD dwHeight )
{
	m_dwWidth	= dwWidth;
	m_dwHeight	= dwHeight;	
	m_pMap		= pMap;
...
bool GQuadTreeIndex::SubDivide(GNode* pNode)
{	
...
void GQuadTreeIndex::UpdateBoundingBox(GNode* pNode)
{	
...
void	GQuadTreeIndex::UpdateObject(GNode* pNode)
{
	for (int iObj = 0; iObj < pNode->m_ObjectList.size(); iObj++)
	{
		float fHeight = m_pMap->GetHeight(	pNode->m_ObjectList[iObj]->m_tBox.vCenter.x, 
							pNode->m_ObjectList[iObj]->m_tBox.vCenter.z);
		pNode->m_ObjectList[iObj]->m_vPosition.y = fHeight;
		pNode->m_ObjectList[iObj]->Update();
	}
}
void	GQuadTreeIndex::ComputeBoungigBox( GNode* pNode )
{
...
GNode* GQuadTreeIndex::CreateNode( GNode* pParentNode, DWORD dwTL,DWORD dwTR,DWORD dwBL,DWORD dwBR )
{	
	GNode* pNode = 0;
...
D3DXVECTOR2	GQuadTreeIndex::GetHeightFromNode(	DWORD dwTL, DWORD dwTR, DWORD dwBL, DWORD dwBR )
{
	assert( m_pMap );
...
int GQuadTreeIndex::UpdateIndexList(GNode* pNode, DWORD dwCurentIndex, DWORD dwNumLevel)
{
	...
	DWORD dwOffset = pow(2.0f, (float)dwNumLevel);
...
// 2, 0 ( 좌, 상 )
// 3, 1 ( 우, 하 )
int  GQuadTreeIndex::SetLodIndexBuffer(GNode* pNode,
	DWORD& dwCurentIndex,
...
bool GQuadTreeIndex::Frame()
{	
	m_DrawPatchNodeList.clear();
	// PreFrame함수 호출 및 오브젝트 컬링
	if( !GQuadTree::Frame() ) return false;
...
int GQuadTreeIndex::UpdateStaticIndexList( GNode* pNode, DWORD dwCurentIndex, DWORD dwLod  )
{
	if( !m_IndexList ) return false;	
	int iNumFaces = 0;
...
DWORD GQuadTreeIndex::GetIndexCounter( DWORD dwData, DWORD dwNumCell )
{	
...
bool GQuadTreeIndex::ComputeStaticLodIndex( int iMaxCells )
{
	m_pdwLodIndexArray.reserve(m_iPatchLodCount+1);	

	// 레벨 0
	GStaticData*  pStaticData = NULL;
	SAFE_NEW( pStaticData, GStaticData );
...
DWORD GQuadTreeIndex::GetIndex( DWORD dwIndex, DWORD dwTL, DWORD dwTR, DWORD dwBL, DWORD dwBR, DWORD dwLod  )
{
	DWORD dwReturn = 0;
	DWORD dwStartRow = dwTL / m_dwWidth;
	DWORD dwStartCol = dwTL % m_dwWidth;
...
HRESULT GQuadTreeIndex::CreateIndexBuffer( GStaticData*  pData )
{	
	HRESULT hr=S_OK;
...
HRESULT GQuadTreeIndex::CreateVertexBuffer(GNode* pNode, DWORD dwCount, PNCT_VERTEX* pvHeightMap)
{	
	HRESULT hr=S_OK;
...
bool GQuadTreeIndex::CreateVertexData(GNode* pNode)
{
	DWORD dwTL = pNode->m_CornerIndex[0];
	DWORD dwTR = pNode->m_CornerIndex[1];
	DWORD dwBL = pNode->m_CornerIndex[2];
...
GQuadTreeIndex::GQuadTreeIndex(void)
{
