...
bool Sample::Render()
{
	...
	for (int iLight = 0; iLight < g_iMaxLight; iLight++)
	{
		...
		m_cbLight2.g_vLightPos[iLight] = D3DXVECTOR4(m_PointLight[iLight].m_vPosition.x,
				...
				m_PointLight[iLight].m_vPosition.z,	
				m_PointLight[iLight].m_fRange);
...
bool Sample::Frame()
{
	...
	m_pMainCamera->m_vCameraPos.y -= 9.8f * fSpeed * g_fSecPerFrame;

	float fHeight =  m_HeightMap.GetHeight(
		m_pMainCamera->m_vCameraPos.x,
		m_pMainCamera->m_vCameraPos.z) + 10.0f;
	if (fHeight > m_pMainCamera->m_vCameraPos.y)
	{
		m_pMainCamera->m_vCameraPos.y = fHeight;
		fSpeed = 1.0f;
	}
	else
	{
		fSpeed += g_fSecPerFrame*10.0f;
	}
	...
	for (int iLight = 0; iLight < g_iMaxLight; iLight++)
	{
		...
		// 지형의 높이값을 얻는다.
		m_PointLight[iLight].m_matWorld._42 = m_HeightMap.GetHeight(
			m_PointLight[iLight].m_vPosition.x,
