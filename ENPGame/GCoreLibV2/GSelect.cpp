...
void GSelect::SetMatrix(D3DXMATRIX* pWorld, D3DXMATRIX* pView, D3DXMATRIX* pProj)
{
	if (pWorld != NULL)
	{
		m_matWorld = *pWorld;
...
//     a                    b                       c
bool GSelect::IntersectRayToSphere(G_SPHERE* pSphere, G_RAY* pRay)
{
...
bool GSelect::IntersectBox(G_BOX* pBox, G_RAY* pRay)
{
	if (pRay == NULL) pRay = &m_Ray;
	D3DXVECTOR3 tmin;
...
bool GSelect::AABBtoRay(G_BOX* a, G_RAY* pRay)
{
	if (pRay == NULL) pRay = &m_Ray;
	float t_min = 0;
...
bool GSelect::OBBtoRay(G_BOX* pBox, G_RAY* pRay)
{
	if (pRay == NULL) pRay = &m_Ray;
...
bool GSelect::ChkOBBToRay(G_BOX* pBox, G_RAY* pRay)
{
	if (pRay == NULL) pRay = &m_Ray;
	float t_min = -999999.0f;
	float t_max = 999999.0f;
...
D3DXVECTOR3 GSelect::GetIntersection()
{
	return m_vIntersection;
}
// 교점을 찾는다.
bool    GSelect::GetIntersection(
	D3DXVECTOR3 vStart,
	D3DXVECTOR3 vEnd,
	D3DXVECTOR3 vNormal,
	D3DXVECTOR3 v0,
...
bool GSelect::ChkPick(D3DXVECTOR3& v0, D3DXVECTOR3& v1, D3DXVECTOR3& v2)
{
	FLOAT fBary1, fBary2;
	FLOAT fDist;
...
bool GSelect::IntersectTriangle(const D3DXVECTOR3& orig, const D3DXVECTOR3& dir,
