...
void GDxShader::Apply(ID3D11DeviceContext*	pd3dContext)
{
...
HRESULT		GDxShader::Load(ID3D11Device* pd3dDevice,
...
void GDxShader::ApplyVS(ID3D11DeviceContext*	pd3dContext, bool bUse)
{
	if (bUse)
		pd3dContext->VSSetShader(m_pVertexShader, NULL, 0);
	else
...
void GDxShader::ApplyHS(ID3D11DeviceContext*	pd3dContext, bool bUse)
{
	if (bUse)
		pd3dContext->HSSetShader(m_pHullShader, NULL, 0);
	else
...
void GDxShader::ApplyDS(ID3D11DeviceContext*	pd3dContext, bool bUse)
{
	if (bUse)
		pd3dContext->DSSetShader(m_pDomainShader, NULL, 0);
	else
...
void GDxShader::ApplyGS(ID3D11DeviceContext*	pd3dContext, bool bUse)
{
	if (bUse)
		pd3dContext->GSSetShader(m_pGeometryShader, NULL, 0);
	else
...
void GDxShader::ApplyPS(ID3D11DeviceContext*	pd3dContext, bool bUse)
{
	if (bUse)
		pd3dContext->PSSetShader(m_pPixelShader, NULL, 0);
	else
...
void GDxShader::ApplyCS(ID3D11DeviceContext*	pd3dContext, bool bUse)
{
	if (bUse)
		pd3dContext->CSSetShader(m_pComputeShader, NULL, 0);
	else
...
HRESULT GDxShader::CompileShaderFromFile(const WCHAR* szFileName, DWORD flags, LPCSTR szEntryPoint, LPCSTR szShaderModel, ID3DBlob** ppBlobOut)
{
	HRESULT hr = S_OK;
...
HRESULT GDxShader::CompileShaderFromFile(const WCHAR* szFileName, LPCSTR szEntryPoint, LPCSTR szShaderModel, ID3DBlob** ppBlobOut)
{
	HRESULT hr = S_OK;
...
HRESULT GDxShader::Load(ID3D11Device* pDevice, const TCHAR* strShaderPath,
...
void GDxShader::ClearShaderResources(ID3D11DeviceContext*  pImmediateContext)
{
	// unbind resources
	...
	STAGE_TYPE type)
{
	...
	if (pLoadShaderFile)
	{
		...
		{
			GDxShader *pPoint = (GDxShader *)(*itor).second;
...
GDxShader* TShaderMgr::GetPtr(INT iIndex)
{
	TemplateMapItor itor = TMap.find(iIndex);
	if (itor == TMap.end()) return NULL;
	GDxShader *pPoint = (*itor).second;
	return pPoint;
}
