...
void GNoiseMap::Set( float fStepSize, float fHurstIndex, float fLacunarity, float fOctaves )
{// 프리퀀시: 주파수, Lacunarity:공극성, 
	...
	SAFE_NEW_ARRAY( m_pExponentArray, double, m_fOctaves+1 );
	float frequency = 1.0;
...
bool GNoiseMap::CreateVertexData()
{
...
////펄린 브라운 모션
////                                       정정리스트 ,  H계수,        진폭                       옥타브
//double GNoiseMap::fBm( float vertex[], double H, double lacunarity, double octaves )
//{
//	double remainder;	
//	double value = 0.0;
//
...
//	return value;
//}
double GNoiseMap::fBM( float vertex[], double fHurstIndex, double fLacunarity, double fOctaves )
{
	...
	{
		vertex[0]	*= fLacunarity;
		vertex[1]	*= fLacunarity;
		vertex[2]	*= fLacunarity;
		Frequency	*= fLacunarity;// 공극성( 주파수의 변화량 )
...
float GNoiseMap::PerlinNoise( float vertex[], double fPersistence, double fOctaves )
{
	float fResult = 0.0f;
	float n = (int)fOctaves - 1;
	...
	{
		//fResult	+= remainder * m_Noise.Noise3( vertex ) * Amplitude;
		fResult		+= remainder * m_Noise.Noise( fV,3 ) * Amplitude;
	}
	return fResult;
}
void GNoiseMap::ReSet()
{
	if( m_iNoiseType == PERLIN_NOISE)
	{
		m_fHurstIndex	= 0.5f;
