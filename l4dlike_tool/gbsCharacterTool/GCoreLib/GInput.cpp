...
//   or     and      xor           xand          not
//   |       &       ^             없음          !   => 비트연산자
BYTE GInput::KeyCheck(BYTE dwKey)
{
	// GetKeyState()		// 동기적
...
bool		GInput::Frame()
{
	...
	ZeroMemory(&g_InputData, sizeof(GBASIS_INPUT_MAP));

	if (m_MouseState[0] >= KEY_PUSH) 	g_InputData.bLeftClick = true;
	if (m_MouseState[0] >= KEY_PUSH) 	g_InputData.bRightClick = true;
	if (m_MouseState[0] >= KEY_PUSH) 	g_InputData.bMiddleClick = true;
